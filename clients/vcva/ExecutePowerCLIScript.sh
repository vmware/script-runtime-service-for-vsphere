...
#login to SRS
srsapikey=$(curl -X POST "https://${srs_address}/api/auth/login" -H "accept: \*/\*" \
			-H "Authorization: Basic ${encodedauthinfo}" -H "content-length: 0" -i -k \
			| grep X-SRS-API-KEY: | awk {'print $2'})
			 
#create runspace
runspaceInfo=$(curl -X POST "https://${srs_address}/api/runspaces" --http1.1 \
...
do
   sleep 3
   response=$(curl -X GET "https://${srs_address}/api/runspaces/${runspaceId}" \
...
curl -X POST "https://${srs_address}/api/script-executions" --http1.1 \
