...
if [ "$EXPORT_ALL_IN_ONE_ARCHIVE" = "one-archive" ];then
   ...
   if [ "$EXPORT_ALL_NEEDED_CONTAINERS" = "export-all-containers" ];then
      ...
      echo "INFO: Exporting All Needed Containers to One Archive '$CONTAINERS_EXPORT_DIR/$ONE_ARCHIVE_NAME'"

      $DOCKER_COMMAND save -o "$CONTAINERS_EXPORT_DIR/$ONE_ARCHIVE_NAME" \
         $CERT_GEN_IMAGE_NAME:$CERT_GEN_IMAGE_VERSION \
      ...
      echo "INFO: Export Essential Only Containers to '$CONTAINERS_EXPORT_DIR/$ONE_ARCHIVE_NAME'"
   ...
   if [ "$EXPORT_ALL_NEEDED_CONTAINERS" = "export-all-containers" ];then
      echo "INFO: Export All Needed Containers to '$CONTAINERS_EXPORT_DIR'"

      $DOCKER_COMMAND save -o "$CONTAINERS_EXPORT_DIR/$PHOTON_IMAGE_NAME-$IMAGE_EXPORT_FILE" $CERT_GEN_IMAGE_NAME:$CERT_GEN_IMAGE_VERSION
      $DOCKER_COMMAND save -o "$CONTAINERS_EXPORT_DIR/$CERT_GEN_PRETTY_IMAGE_NAME-$IMAGE_EXPORT_FILE" $NGINX_IMAGE_NAME:$NGINX_IMAGE_VERSION
      $DOCKER_COMMAND save -o "$CONTAINERS_EXPORT_DIR/$NGINX_PRETTY_IMAGE_NAME-$IMAGE_EXPORT_FILE" $PHOTON_IMAGE_NAME:$PHOTON_IMAGE_VERSION
   else
      echo "INFO: Export Essential Only Containers to '$CONTAINERS_EXPORT_DIR'"
   fi
