...
using VMware.ScriptRuntimeService.Setup.VCRegistration.Exceptions;
using VMware.ScriptRuntimeService.SsoAdmin;

namespace VMware.ScriptRuntimeService.Setup.VCRegistration {
   public class VCRegistrator {
      ...
      private void UnregisterInternal(
         ...
         var setupServiceSettings = SetupServiceSettings.FromStsSettings(new Setup.StsSettings {
      ...
      private X509Certificate2 GetOrGenerateCertificate<T>(string certificatePath, string commonName, string certificateName) where T : ISelfSignedCertificateGenerator {
         ...
         if (!string.IsNullOrEmpty(certificatePath) &&
            ...
            ISelfSignedCertificateGenerator certGen = (ISelfSignedCertificateGenerator) Activator.CreateInstance(
               typeof(T),
               _loggerFactory,
               commonName,
               _configWriter);
