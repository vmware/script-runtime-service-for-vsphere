...
namespace VMware.ScriptRuntimeService.RunspaceClient.Bindings.Client {
   ...
   public class Configuration : IReadableConfiguration {
      ...
      public static readonly ExceptionFactory DefaultExceptionFactory = (methodName, response) =>
      {
         ...
         if (status >= 400) {
            var errorResponse = (ErrorResponse)new ApiClient().Deserialize(response, typeof(ErrorResponse));
         ...
         if (status == 0) {
            return new ApiException(status,
                string.Format("Error calling {0}: {1}", methodName, response.ErrorMessage), null);
         }
         return null;
      };
